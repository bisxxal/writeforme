'use client'
import { createWritterProfile, userProfile } from '@/actions/user.action'
...
const EditPage = () => {
    ...
    const handleSubmit = (formData: FormData) => {
        ...
        try {
            ...
            console.log(description, pagePrice, digramsPrice)
    ...
    return (
        <div className=' w-full px-10 pb-20'>
            ...
            <h1 className=' text-2xl font-bold text-center mb-3'>My work</h1>

            <form action={handleSubmit}>

                <label className=' mt-5 block font-medium text-xl'>Description</label>
                <input defaultValue={data?.user?.description as string} name='description' className='w-full card p-2 rounded-lg' type="text" />
                ...
                <div className='mt-4 w-full between'>Fixed minimum rate per page <div className=' gap-2 center '>
                    <input type="number" defaultValue={data?.user?.pagePrice as number} required className=' w-[32px] rounded-2xl px-1 card center gap-2' name='page' placeholder='12' /> <Pencil size={20} /> </div>
                </div>

                <div className='mt-4 w-full between'>Fixed minimum rate per diagram  <div className=' gap-2 center '>
                    <input type='number' defaultValue={data?.user?.digramsPrice as number} required className='w-[32px] rounded-2xl px-1 card  center gap-2' name='digram' placeholder='12' /> <Pencil size={20} /> </div>
                </div>
